...
int sodium_compare(const ubyte[] b1_, const ubyte[] b2_) @nogc nothrow pure @trusted
{
    ...
    if      (b1_.length < b2_.length && !sodium_is_zero(b2_[b1_.length..$]))
        ...
        return 1;
    return  sodium_compare(b1_.ptr, b2_.ptr, min(b1_.length, b2_.length)); // __attribute__ ((warn_unused_result));
}
...
//sodium_is_zero (overload)
    ...
    {   // testing the overload
        ...
        assert(sodium_compare(c, d) == -1);
        c[5] = 1;
        assert(sodium_compare(c, d) ==  1); // if (b1_.length > b2_.length && !sodium_is_zero(b1_[b2_.length..$])) return  1;
        d = c.dup;
        d ~= 1;
        assert(sodium_compare(c, d) == -1); // if (b1_.length < b2_.length && !sodium_is_zero(b2_[b1_.length..$])) return -1;
        assert(!sodium_memcmp(c, d));
